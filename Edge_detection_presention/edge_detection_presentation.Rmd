---
title: "Edge Detection from Scratch Presentation"
...
edge_detect <- function(image_name, type = c("jpeg", "png"),filter=c("Prewitt", "Sobel","Scharr")){
  ...
  # define filters 
  if (filter == "Prewitt"){
    vertical_filter <- flipdim(matrix(c(1,1,1, 0,0,0,-1,-1,-1),3,3),1)
    vertical_filter <- flipdim(vertical_filter,2)
    horizontal_filter <- t(vertical_filter)
  }else if (filter == "Sobel"){
    vertical_filter <- flipdim(matrix(c(1,2,1, 0,0,0,-1,-2,-1),3,3),1)
    vertical_filter <- flipdim(vertical_filter,2)
    horizontal_filter <- t(vertical_filter)
  }else if (filter == "Scharr"){
    vertical_filter <- flipdim(matrix(c(3,10,3, 0,0,0,-3,-10,-3),3,3),1)
    vertical_filter <- flipdim(vertical_filter,2)
    horizontal_filter <- t(vertical_filter)
  }
  else{
    if (class(filter) == c("matrix","array") & dim(filter)[1]==dim(filter)[2]){
      vertical_filter <- flipdim(filter,1)
      vertical_filter <- flipdim(vertical_filter,2)
      horizontal_filter <- t(vertical_filter)
    }
    else{
...
Gaussian_blur_1 <-function(size, sigma){
  k <- size %/% 2
...
gaus_blur_ope <- function(img, filter){
  ...
  # Zero padding 
  zeros_row <- zeros(1,m)
  padded_row_img <- rbind(rbind(rbind(zeros_row,rbind(zeros_row, img[,,1])), zeros_row),zeros_row)
  zeros_col <- zeros(dim(padded_row_img)[1],1)
  padded_col_img <- cbind(cbind(cbind(zeros_col,cbind(zeros_col, padded_row_img)), zeros_col),zeros_col)
  paded_img <- array(rep(padded_col_img ,c), dim = c(dim(padded_col_img)[1],dim(padded_col_img)[2],c))
...
# writeJPEG(gaus_blur_ope(black_white(img), Gaussian_blur_1(5,1.4)), target ="smoothed_monarch_in_may.jpeg")
...
convolu_ope <- function(img, filter=c("Prewitt", "Sobel","Scharr")){
  ...
  #Zero padding 
  zeros_row <- zeros(1,m)
  padded_row_img <- rbind(rbind(zeros_row, img[,,1]), zeros_row)
  zeros_col <- zeros(dim(padded_row_img)[1],1)
  padded_col_img <- cbind(cbind(zeros_col, padded_row_img), zeros_col)
  paded_img <- array(rep(padded_col_img ,c), dim = c(dim(padded_col_img)[1],dim(padded_col_img)[2],c))
  # define filters 
  if (filter == "Prewitt"){
...
# img <- readPNG("monarch_in_may.png")
# smooth_img <- gaus_blur_ope(black_white(img), Gaussian_blur_1(5,1.4))
