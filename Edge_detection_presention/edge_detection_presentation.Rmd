...
# Edge detection through convolution operation

**In this presentation, I'll show how to get the edge image on the right with convolution operation.**
```{r echo=FALSE, fig.align='default', fig.show='hold', out.width="30%"}
knitr::include_graphics(c("minions2.jpeg","edge_img_example.jpeg"))
```

**First of all, what is convolution operation?**

Let's say we have a 6x6 matrix and a 3x3 filter. To perform convolution operation, we would loop over the 6x6 matrix with our filter. For example, we'll first locate a 3x3 matrix on the 6x6 matrix, and we are going to get the sum of the element-wise products of the filter and the 3x3 matrix we located. We repeat this process until we looped through the entire matrix. 

![Convolution operation example](Convolution_operation_example.PNG)



![Convolition Operation in action](convol_op_act.gif)
...
**Now, we know what convolution operation is, let's get started on detecting the edges.**
...
3. Pick a filter 
4. Apply convolution operation with a nested for loop.
...
### Step 4: Apply convolution operation with a nested for loop.
...
# writeJPEG(edge_detect("minions_new.png", type = "png", filter = "Prewitt"), target ="prewitt_minions2.png")
# writeJPEG(edge_detect("minions_new.png", type = "png", filter = "Sobel"), target ="sobel_minions2.jpeg")
# writeJPEG(edge_detect("minions_new.png", type = "png", filter = "Scharr"), target ="scharr_minions2.jpeg")
...
Gaussian_filter <-function(size, sigma){
  k <- size %/% 2
  # Xs are centered around zero instead of starting from 0
  # one of the reason is that when starting from 0, the resulting image is too dark
...
convolu_ope <- function(img, filter=c("Prewitt", "Sobel","Scharr")){
  ...
  # convolution operation with a nested for loop
  for (row in 1:n){
    for (col in 1:m){
      local <- paded_img[row:(row+k-1), col:(col+k-1),1]
      horiz_score <- sum(horizontal_filter*local)
...
Each pixel in the image has a color value and angle value (we calculated the angle value along with the convolution operation, it equals to $arctan(\frac{\text{hori_score}}{\text{-verti_score}})$)
...
# Face Recognition and Object Detection Implementation 
